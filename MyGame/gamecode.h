...
{
private:
	enum GameState{MENU, PAUSED, RUNNING, GAMEOVER,DEAD};
	...
	int framesToFreeze;		
	int finalScore;					// gets player's final score
...
public:
   ...
   // which is currently a basic placeholder
	...
	// Called each frame after player has died
	ErrorType DeadMenu();
   ...
   // This will be used by the gameplay programmer to clean up
	...
	// Called when a big hit/explosion happens in game
	void StopHit();


	// Called when player loses all lives
	void PlayerDead(int score);
};
